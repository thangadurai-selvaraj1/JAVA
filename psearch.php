...
<?php
    ...
    if($numrow!==0)
   {  echo "<tbody>";
   	echo "<center><table border='5px' cellpadding='3px' cellspacing='3px'><tr><th>NAME</th><th>Address</th><th>DISTRICT</th><th>STATE</th><th>PINCODE</th><th>POST ID</th><th>TYPE</th><th>UPDATE</th>";
...
while($row=mysqli_fetch_array($new)) {         
            ...
            echo "<td>".$row['pin']."</td>";  
                ...
                echo "<td>".$row['upid']."</td>"; 
   echo "<td>".$row['type']."</td>";   
            echo "<td><a class='up' href=\"edit.php?id=$row[id]\" >Edit</a> | <a class='up' href=\"delete.php?id=$row[id]\" onClick=\"return confirm('Are you sure you want to delete?')\">Delete</a></td>";        
        }
